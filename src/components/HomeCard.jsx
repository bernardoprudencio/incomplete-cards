...
export default function HomeCard({ time, service, address, petNames, petImages, buttonLabel, disabled = false, onClick }) {
  return (
    <div onClick={onClick} style={{
      background: colors.background.primary, borderRadius: 8, boxShadow: shadows.low,
      padding: '12px 20px', cursor: onClick ? 'pointer' : 'default',
    }}>
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '8px 0' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ ...textStyles.text200Semibold, color: colors.text.primary, margin: 0 }}>{time}</p>
          <p style={{ ...textStyles.text100, color: colors.text.secondary, margin: 0 }}>{service}</p>
        </div>
      ...
      <div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0' }}>
          ...
          <span style={{ fontFamily: typography.fontFamily, fontSize: 14, color: colors.text.primary }}>{address}</span>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '8px 0' }}>
          <span style={{ flexShrink: 0 }}><PawIcon /></span>
          <span style={{ fontFamily: typography.fontFamily, fontSize: 14, color: colors.text.primary }}>{petNames}</span>
        </div>
